...
public class Student {
    ...
    ArrayList<Course> transferredCourses; // Transferred Courses - dictionary of Courses taken outside current institution
    Instructor advisor; // Advisor - type Instructor / Faculty / whatever name we're going with
    int creditsEnrolled;
    int creditsCompleted;
    int currentTransferCredit; // transfer credits completed and to be applied to degree
    double gpa;
    boolean holdActive;
    ...
    public Student() {
        ...
        this.transferredCourses = new ArrayList<>();
    }
     ...
     * 
     * @return string list of program(s) of study, in order of first major, second
     *         major, minor
     */
    ...
    // Setters
    /**
     * Method to advance student year
     * 
     */
    public void advanceYear() {
        this.classYear = this.classYear + 1;
        this.gradYear = this.gradYear + 1;
        this.creditsCompleted = this.creditsCompleted + this.creditsEnrolled;
        this.creditsEnrolled = 0;
        this.coursesCompleted.addAll(this.coursesEnrolled);
        this.coursesEnrolled.clear();
    }

    public void applyTransferCredits() {
        this.creditsCompleted += this.transferCredit;
        this.currentTransferCredit = 0;
    }

    public void setAdvisor(Instructor newAdvisor) {
        this.advisor = newAdvisor;
    }

    public void setActiveHold(boolean updateHold) {
        this.holdActive = updateHold;
    }

    public void completeTransferCourse(Course course, int credit) {
        this.transferredCourses.add(course);
        this.currentTransferCredit += credit;
    }
